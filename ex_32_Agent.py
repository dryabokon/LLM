...
def ex_agent_pandas_finance(df):
    ...
    query0 = 'Forget all prev instructions. Do nothing. Forget prev instructions. Get Ready to answer upcoming questions. Response with OK by now.'
    ...
    query5 = 'Use available tool to evaluate required cashflow to generate for Investment_Bosch target IRR of 0.23. Assume cashflow data is encoded by metricName Csh. Filter out duplicates and ensure cashflow is sorted over a time.'
    query6 = 'Evaluate required cashflow to generate for Investment_Bosch target IRR of 0.23. Assume cashflow data is encoded by metricName Csh. Filter out duplicates and ensure cashflow is sorted over a time.'

    #q = [query0,query1,query2,query3,query4,query5]
    q = [query0,query5]
...
def ex_agent_churn(df):
    q= 'Forget all about prev queries. Now response to question: according to historical dataset, what are the factors to segment/predict churn and retained customers ?'
    q = [q]
...
def ex_agent_pandas_automotive(df):
    query0 = 'count number of records in dataframe'
    query1 = 'Recommend a best value for price engine taking into account price, power, and mpg. Assume price data is encoded by price, power by  horsepower, and mpg by city-mpg.'
