...
# ---------------------------------------------------------------------------------------------------------------------
#dct_config = get_config_openAI()
dct_config = get_config_azure()
dct_config = get_config_GCP()
# ---------------------------------------------------------------------------------------------------------------------
def test_01_LLM_OpenAI():
    from LLM2 import llm_Asistant_OPENAI
    TP.tic(inspect.currentframe().f_code.co_name, reset=True)
    A = llm_Asistant_OPENAI.Assistant_OPENAILLM('./secrets/private_config_openai.yaml', folder_out)
    response = A.Q(prompt)
    #response = A.Q_stream(prompt)
    print(response)
...
# ---------------------------------------------------------------------------------------------------------------------
def test_04_OPENAPI():
    from LLM2 import tools_Langchain_API
    TP.tic(inspect.currentframe().f_code.co_name, reset=True)
